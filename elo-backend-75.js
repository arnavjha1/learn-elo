...
function problemWrong() {
  updateRating(
    Math.round(
      array[Math.floor(progress / 75)][0] * ratingJump
    )
  );
...
function problemCorrect() {
  if (progress < 5000) {
    updateRating(array[Math.floor(progress / 75)][1] * ratingJump);
  }
  problemsCorrect++;
...
function loadNewQuestion() {
  ...
  questionsSolved++;
  problem = Math.round(progress / 75) - 1;
...
function gradeProblem(x) {
  ...
  acc.innerHTML =
    Math.round((problemsCorrect * 75) / (questionsSolved + 1)) + "%";
